...
class UgcController extends Controller
{
    ...
    {
    	$data = Ugc::orderby('api_id','asc')->paginate(6);
    	return view('rest.ugc.index')->with('data', $data);
    }
    
    //post.rest/userpicture/create 添加接口
    public function create()
    {
    	return view('rest.ugc.add');
    }
    ...
    {  	
        ...
        if ($validator->passes())
        {
         	$ret = Ugc::create($input);  
         	if ($ret)
         	{
         		return redirect('rest/ugc');
         	} 
         	else
         	{
    ...
    {
    	$data = Ugc::find($api_id);
    	return view('rest.ugc.edit', compact('data'));
    }
    //put.rest/userpicture/[user] 更新单个用户信息
    public function update($api_id)
    {
    	$input = Input::except('_token', '_method');
    	$ret = Ugc::where('api_id', $api_id)->update($input);
    	if ($ret)
    	{
    		return redirect('rest/ugc');
    	}
    ...
    {
    	$ret = Ugc::where('api_id',$api_id)->delete();
        if ($ret)
        {
            $data = [
                'status' =>0,
